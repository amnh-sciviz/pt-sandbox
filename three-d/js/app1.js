...
function addMolecule(){
  ...
  if (currentIndex < 0) {
    rotationAmount = Math.atan(Math.sqrt(2));
    rotationAxis = new THREE.Vector3(1, 0, -1).normalize();
    rotateGroup.applyMatrix(new THREE.Matrix4().makeRotationAxis(rotationAxis, rotationAmount));
    rotationAmount = Math.atan(Math.sqrt(2)) / 3.6;
